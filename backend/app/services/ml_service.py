...
class MLRiskService:
    ...
    def prepare_data(self, events):
        ...
        for col in ['email_opened', 'link_clicked', 'credential_submitted']:
            if col not in features.columns:
                features[col] = 0
        
        features['risk_label'] = np.where(features['credential_submitted'] > 0, 2, 
    ...
    def train_on_history(self, historical_data):
        ...
        X = features[['email_opened', 'link_clicked', 'credential_submitted']]
    ...
    def predict_risk_score(self, user_stats):
        if not self.is_trained:
            score = (user_stats['email_opened'] * 0.1 + 
                     user_stats['link_clicked'] * 0.4 + 
                     user_stats['credential_submitted'] * 0.5)
            return float(min(score, 1.0))

        X = pd.DataFrame([user_stats])
        X = X[['email_opened', 'link_clicked', 'credential_submitted']]
